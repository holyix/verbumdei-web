...
<section class="quest-hero">
    <div>
        <p class="eyebrow">Verbum Quest</p>
        <h1>Eras of Scripture</h1>
        <p class="lead">
            Browse the era summaries and step into an episode when you are ready to begin a quiz.
        </p>
    </div>
    <div class="hero-card">
        <p class="hero-label">Quiz Scope</p>
        <p class="hero-body">
            Episodes are the only quiz entry point in this version. Era cards are for navigation.
        </p>
    </div>
</section>
...
<section class="card-grid">
    {#if data.eras.length === 0}
        <div class="empty-state">
            <h2>No eras yet</h2>
            <p>Once eras are available, they will appear here with episode counts.</p>
        </div>
    {:else}
        {#each data.eras as era}
            <button class="era-card" type="button" on:click={() => openEra(era.id)}>
                ...
                <span class="era-action">View Era</span>
            </button>
        {/each}
    {/if}
</section>
